...
declare const org: {
  nativescript: {
    Benchmarks: {
      ...
      voidMethod(): void;
      passAndReturnString(str: string): string;
      returnString: () => string;
      returnInt: () => number;
...
export class HelloWorldModel extends Observable {
  ...
  get engine(): string {
    //@ts-ignore
    return typeof __engine === "string" ? __engine : "V8-Current";
  }
  ...
  private startBenchmark() {
    ...
    //@ts-ignore
    const benchmarks = new org.nativescript.Benchmarks();

    message +=
      measure("Void Method on instance", () => {
        for (var i = 0; i < 1e6; i++) {
          benchmarks.voidMethod1();
        }
      }) + "\n";

    message +=
      measure("Field on instance", () => {
        let f;
        for (var i = 0; i < 1e6; i++) {
          f = benchmarks.InstanceField;
        }
      }) + "\n";

    message +=
      measure("Int Field on instance", () => {
        let f;
        for (var i = 0; i < 1e6; i++) {
          f = benchmarks.IntFieldInstance;
        }
      }) + "\n";
    ...
    message +=
      measure("multiply(a,b)", () => {
        for (var i = 0; i < 1e6; i++) {
          org.nativescript.Benchmarks.multiply(3, 2);
        }
      }) + "\n";

    message +=
      measure("Pass and return string", () => {
        const method = org.nativescript.Benchmarks.passAndReturnString;
        for (var i = 0; i < 1e6; i++) {
          method("hello world");
        }
      }) + "\n";
    ...
    message +=
      measure("Return a Boolean Array", () => {
        let res;
        for (var i = 0; i < 10000; i++) {
    ...
    message +=
      measure("Pass a Date Object", () => {
        for (var i = 0; i < 10000; i++) {
          org.nativescript.Benchmarks.passDateObject(new java.util.Date());
        }
      }) + "\n";

    message += measure("Invoke void callback from java", () => {
      const callback = new org.nativescript.BenchmarkCallback({
        onCallback() {},
      });
      org.nativescript.Benchmarks.invokeCallback(callback);
    });
